...
### 4.2 Agent 注册中心 (agent/registry.py)
...
### 4.3 LLM 客户端 (agent/provider/llmclient.py)
...
### 4.4 对话管理 (agent/core/conversation.py)
...
### 4.5 技能系统 (agent/skill/skill_loader.py)
...
### 4.6 工具适配器 (agent/tool/tooladapter.py)
...
过滤实现位置：
- `agent/main.py` - `_filter_tools()` 和 `_filter_skills()` 方法
- `agent/skill/skill_loader.py` - `get_skill_list_info(patterns)` 方法

### 4.7 MCP 管理器 (agent/mcp/mcp_manager.py)
...
### 4.9 Bot 网关 (agent/server/gateway.py)
