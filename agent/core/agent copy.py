...
class Agent:
    ...
    def _get_core_tool_names(self) -> List[str]:
        ...
        # 检查缓存是否存在且有效
        if hasattr(self, '_core_tool_names_cache') and self._core_tool_names_cache is not None:
            return self._core_tool_names_cache
        ...
        try:
            ...
            data = self.memory_system.tool_store._read_tool_index()
            tools = data.get("tools", [])
            core_names = [t["name"] for t in tools if t.get("category") == "core" and t.get("enabled", True)]
            
            # 缓存结果
            self._core_tool_names_cache = core_names if core_names else default_core_tools
            return self._core_tool_names_cache
        except Exception as e:
    ...
    def _get_core_tools(self) -> List[Dict[str, Any]]:
        ...
        logger.debug(f"[_get_core_tools] tool_adapter.tools 中的工具: {list(self.tool_adapter.tools.keys())}")
        core_tools = []
    ...
    def _prepare_tools_for_ability_match(self, required_abilities: List[str]) -> List[Dict]:
        ...
        """
        core_tools = self._get_core_tools()
        logger.debug(f"[工具准备] _get_core_tools 返回 {len(core_tools)} 个工具, memory_system={self.memory_system is not None}")
